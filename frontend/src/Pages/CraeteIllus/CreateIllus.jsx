            ...
            <form onSubmit={createStory} className="flex flex-col  gap-5 shadow-[0_0_10px_gray] rounded-lg p-5 w-1/2">
                <div className="flex items-start justify-between gap-5">
                    <div className="flex flex-col items-center gap-5 w-1/3">
                        <label htmlFor="avatar" className=""><img className="h-36 rounded-sm" src={previewImage ? URL.createObjectURL(previewImage):uploadImage} alt="" /></label>
                        <input required onChange={(e) => setPreviewImage(e.target.files[0])} type='file' id="avatar" className="hidden" />
                        <input required onChange={(e)=>handleUserInput(e)} name="title" type="text" className="bg-white w-44 p-2.5 rounded-lg outline-none" placeholder="Title" />
                    </div>
                    <div className="w-2/3">
